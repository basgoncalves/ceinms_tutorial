...
ceinms_install_path = os.path.join(current_dir, 'ceinms', 'CEINMS.exe')
...
SUBJECT = 'PC006'  # Subject ID
TRIAL_NAME = 'trial2_l1'

C3D = '{subject_name}.c3d'
MARKERS = 'markers_experimental.trc'
GRF = 'Visual3d_SIMM_grf.mot'
EMG_CSV = 'processed_emg_signals.csv'
EMG_MOT = 'processed_emg_signals.mot'
IK = 'Visual3d_SIMM_input.mot'
ID = 'inverse_dynamics.sto'
SO_FORCE = 'Results_SO_and_MA{subject}_StaticOptimization_force.sto'
SO_ACTIVATION = SO_FORCE.replace('force', 'activation')
JRA = 'joint_reacton_loads.sto'

IK_COLUMNS = ["hip_flexion_leg", "hip_adduction_leg", "hip_rotation_leg", "knee_angle_leg", "ankle_angle_leg"]
ID_COLUMNS = ["hip_flexion_leg" + "_moment", "hip_adduction_leg" + "_moment", "hip_rotation_leg" + "_moment", "knee_angle_leg" + "_moment", "ankle_angle_leg" + "_moment"]
EMG_SIGNAL_LABELS = ['GLTMED', 'RF', 'ADDLONG', 'ST', 'TA', 'GM']

#%% FUNTIONS CREARE (DO NOT CHANGE)
def import_json_file(jsonFilePath):
    ...
    try:
        ...
        print('Error reading ' + jsonFilePath)
        data = dict()
    ...
    def __init__(self, path):
        ...
        try:
            if self.extension == '.csv':
                ...
                self.data = import_json_file(path)
...
class Trial:
    '''
    Class for a single trial in the simulation.
    Input: trial_path -> str()
    Path to the trial folder containing all files related to the trial.
    
    ik -> Inverse Kinematics results
    id -> Inverse Dynamics results
    ...
    def __init__(self, trial_path):        
        ...
        self.name = os.path.basename(self.path)
        self.subject = os.path.basename(os.path.dirname(self.path))
        self.c3d = os.path.join(os.path.dirname(self.path), C3D.format(subject_name=self.subject))
        self.markers = File(os.path.join(self.path, MARKERS))
        self.grf = File(os.path.join(self.path, GRF))
        self.emg_csv = File(os.path.join(self.path, EMG_CSV))
        self.emg = File(os.path.join(self.path, EMG_MOT))
        self.ik = File(os.path.join(self.path, IK))
        self.id = File(os.path.join(self.path, ID))
        self.so_force = File(os.path.join(self.path, SO_FORCE.format(subject=self.subject))) 
        self.so_activation = File(os.path.join(self.path, SO_ACTIVATION.format(subject=self.subject)))
    ...
    def csv_to_mot(self):
        ...
        emg_data = pd.read_csv(self.emg_csv)
    ...
    def exportC3D(self):
        return c3d_analog_export(self.c3d, emg_labels='all', replace=True)
    ...
    def to_json(self):
        save_json_file(self.__dict__, jsonFilePath = self.settings_json)
        print('settings.json created in ' + self.settings_json)
    
    def run_IK(self, setup_file):
        '''
        ...
        '''
        model = osim.Model(self.ik.path)
        model.initSystem()
        
        # Create the inverse kinematics tool
        ikTool = osim.InverseKinematicsTool(setup_file)
        ikTool.setModel(model)
        ikTool.setMarkerDataFileName(self.markers.path)
        ikTool.setOutputMotionFileName(self.ik.path)
        ikTool.setResultsDir(os.path.dirname(self.ik.path))
...
class openSim:
    ...
    def plot_single_trial(self, show = False):
        ...
        time_normalized = np.linspace(0, 100, 101)  
...
if __name__ == "__main__":
    ...
    # Load data (all participants in the)
    os_analysis = openSim(legs=['r','l'], subjects=SUBJECT_LIST, trials_to_load=TRIAL_LIST, trial_number=1)

    # clear output
    clear_output()
    
    # Loop and check if files exist for each subject
    for i_subject in SUBJECT_LIST:
        TRIAL_LIST = os_analysis.subjects[i_subject].keys()
        for TRIAL_NAME in TRIAL_LIST:
                leg = TRIAL_NAME[-2]
                try:
                    trial_path = os_analysis.subjects[i_subject][TRIAL_NAME].path
                    subject_path = os.path.dirname(trial_path)
                    model = os.path.join(subject_path, f'{i_subject}_v3.osim')
                    
                    # check if IK file exists
                    if not os.path.exists(os_analysis.subjects[i_subject][TRIAL_NAME].ik.path):
                        try:
                    ...
                    os_analysis.subjects[i_subject][TRIAL_NAME].create_ceinms_files(osim_model_path=model,
                                                                                leg=leg,
                                                                                input_signals=EMG_SIGNAL_LABELS)
                except:
                    print(f"Error creating CEINMS files for {i_subject} - {TRIAL_NAME} - {leg}")
                    
                print(f"CEINMS files created for {i_subject} - {TRIAL_NAME} - {leg}")
    ...
    print(f'Subject and trial: {SUBJECT} - {TRIAL_NAME}')
    #check times
    breakpoint()
    print(f'time range IK {os_analysis.subjects[SUBJECT][TRIAL_NAME].ik.time_range}')
    print(f'time range ID {os_analysis.subjects[SUBJECT][TRIAL_NAME].id.time_range}')
    print(f'time range EMG {os_analysis.subjects[SUBJECT][TRIAL_NAME].emg.time_range}')
    
    print(str(command))
    
    # Ask user if wants to continue with calibration
    proceed = input("Do you want to proceed with calibration? (y/other): ").strip().lower()
    if proceed != 'y':
        print("Exiting without creating CEINMS files.")
        exit()
